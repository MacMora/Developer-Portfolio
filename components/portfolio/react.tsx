...
import { SquareArrowOutUpRightIcon, Github } from "lucide-react";
...
const ReactProjects: React.FC = () => {
  ...
  const totalPages: number = Math.ceil(dataReact.length / itemsPerPage);
  ...
  const paginatedData: ProjectItem[] = dataReact
    .toReversed()
    ...
    <div className="py-[80px] justify-items-center">
      <div className="w-full">
        <h2 className="text-[#7828c8] dark:text-[#9455d3] font-bold text-xl leading-tight text-start md:text-2xl">
          {t("portfolio.projects.react.content.title")}
        </h2>
        <p className="text-sm md:text-lg my-2">
          {t("portfolio.projects.react.content.description")}
        </p>
        <ul className="text-sm md:text-lg list-disc pl-5 space-y-2">
          <li>{t("portfolio.projects.react.content.main.1")}</li>
          <li>{t("portfolio.projects.react.content.main.2")}</li>
          <li>{t("portfolio.projects.react.content.main.3")}</li>
        </ul>
      </div>
          ...
          {paginatedData.map((data: ProjectItem) => (
            <Card key={data.id} isHoverable={true} shadow="lg">
              ...
              <CardFooter className="flex flex-col py-[20px] gap-2 text-small">
                <h3 className="w-full text-xl font-bold text-left">
                  {data.title}
                </h3>
                ...
                <div className="flex flex-row gap-4 w-full align-end pt-[20px]">
                  {/*<Link href={data.urlGithub} target="_blank" rel="nofollow">
                    <Button
                      color="default"
                      variant="light"
                      endContent={
                  ...
                  <Link href={data.urlWeb} target="_blank" rel="nofollow">
                    <Button
                      ...
                      variant="flat"
                      endContent={
                        <SquareArrowOutUpRightIcon width={15} height={15} />
                      }
                    >
                      {t("buttons.visit")}
                    </Button>
          ...
          <ModalContent className="z-100 overflow-auto">
            {(onClose) => (
              <>
                ...
                <Link href={selectedProject.urlGithub} target="_blank" rel="nofollow">
                    <Button
                      color="default"
                      variant="light"
                      endContent={
